...
private fun App(onVolumeShutter: ((() -> Unit)?) -> Unit) {
    ...
    // Only activeMode lives at this level. The elapsed clock and the level meter are
    // read inside the screens that show them, so a per-second tick cannot recompose
    // the navigation bar and every other destination.
